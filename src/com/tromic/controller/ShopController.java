...
public class ShopController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int totalPage = productService.countPage(PAGE_SIZE);
		
		List<Product> listProduct = productService.getAllPagging(pageIndex, PAGE_SIZE);
		List<Category> listCategory = categoryService.getAll();
		List<Supplier> listSupplier = supplierService.getAll();
		
		int countAllProduct = productService.getAll().size();
		int countFoundProduct = productService.getAll().size();
		...
		request.getRequestDispatcher(Constant.Path.SHOP).forward(request, response);
	}
